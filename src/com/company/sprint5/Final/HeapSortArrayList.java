...
public class HeapSortArrayList {
    ...
    public static void heapSort(ArrayList<Participant> participants) {
        ...
        heap.add(new Participant("fake", 0, 0));
        for (int i = 1; i < participants.size() ; i++) {
            heapAdd(heap, participants.get(i));
        }
        ArrayList<Participant> sortedParticipants = new ArrayList<>();
        sortedParticipants.add(new Participant("fake", 0, 0));
        while (heap.size() > 0) {
            sortedParticipants.add(popMax(heap));
        }
        participants.clear();
    ...
    public static void main(String[] args) throws Exception {
        ...
        participants.add(new Participant("fake", 0, 0));
        for (int i = 0; i < n; i++) {
